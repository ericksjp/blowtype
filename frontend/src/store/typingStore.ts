...
const splitContentIntoPages = (content: string, wordsPerPage: number = 150): string[] => {
...
export const useTypingStore = create<TypingState>()(
  subscribeWithSelector((set, get) => ({
    ...
    setSelectedChapter: (chapter) => set(() => {
      const newText = chapter ? get().getPageText(0) : '';
      return {
    ...
    setCurrentPageIndex: (pageIndex) => set(() => {
    ...
    updateStats: (wpm, accuracy) => set((state) => ({
      stats: { ...state.stats, wpm, accuracy }
    })),
    
    completeCurrentPage: () => set((state) => {
      const { selectedBook, selectedChapter, currentPageIndex } = state;
    ...
    goToNextChapter: () => {
      ...
      const currentChapterIndex = selectedBook.capitulos.findIndex(c => c.id === selectedChapter.id);
      ...
      if (nextChapter) {
        set(() => ({
          ...
          stats: {
            ...get().stats,
            totalChaptersCompleted: get().stats.totalChaptersCompleted + 1
          }
        }));
        return true;
      }
